...
/*public class Solution {
    ...
    public static long[] getNumbers(long N) {
        final int maxResultCapacity = 50 * 1_000_000 / 8;
        long[] result = new long[maxResultCapacity];

        String numberText = "";
        long sum = 0L;
        final int ansiShift = 48;
        ...
        for (int i = 1; i < N; i++)
        {
            ...
            powValue = numberText.length();
            sum = 0L;
            isLarger = false;

            for (int j = 0; j < numberText.length(); j++)
            {
                digit = numberText.charAt(j) - ansiShift;
                sum+= Math.pow(digit,powValue);
            ...
            if(false == isLarger)
            {
                if(sum == i)
                {
                    ...
                    if(indexResult == maxResultCapacity) // array result is full
                        break;
                }
            }
        }
        Arrays.sort(result,0,indexResult);

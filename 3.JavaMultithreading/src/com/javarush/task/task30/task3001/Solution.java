...
/*public static Number convertNumberToOtherNumberSystem(Number number, NumberSystem expectedNumberSystem) {
        ...
        int stringSize = digit.length();
        long decimal = 0;
        long decimalValue = 0;

            // convert to decimal
            for (int degree = stringSize - 1, charIndex = 0; degree >= 0; degree--, charIndex++)
            {
                char ch = digit.charAt(charIndex);
                int numeral = (int) ch > 96 ? (int) ch - 87 : Character.getNumericValue(ch); // 'a' has ANSI code 97

                if(numeral >= numberSystemIntValue)
                    throw  new NumberFormatException();

                decimalValue += numeral * Math.pow(numberSystemIntValue,degree);
            }

            // convert to expectedNumberSystem
        int expectedNumberSystemIntValue = expectedNumberSystem.getNumberSystemIntValue();
            StringBuilder expectedValue = new StringBuilder();

            while (true)
            {
                int numeral = (int) (decimalValue % expectedNumberSystemIntValue);

                char ch = expectedNumberSystemIntValue > 10 ? (char) (numeral + 87) : Character.forDigit(numeral, 10);

                expectedValue.insert(0,ch);
                decimalValue /= expectedNumberSystemIntValue;

                if(decimalValue < expectedNumberSystemIntValue)
                {
                    ch = expectedNumberSystemIntValue > 10 ? (char) (decimalValue + 87) : Character.forDigit((int) decimalValue, 10);
                    expectedValue.insert(0,ch);
                    break;
                }
            }
        Number result = new Number(expectedNumberSystem, expectedValue.toString());

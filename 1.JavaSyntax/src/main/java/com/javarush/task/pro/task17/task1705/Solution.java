...
public class Solution {
    ...
    public static void runWorkingProcess() {
        for (var employer : astronauts)
            if (employer instanceof Human)
                pilot((Human) employer);
            else if (employer instanceof Dog)
                createDirection((Dog) employer);
            else if (employer instanceof Cat)
                research((Cat) employer);
    }
